...
function publishCards() {
  const emps = getEmployees();
  if (emps.length === 0) {
    showToast('No employees to publish. Add employees first.', 'error');
    return;
  }

  const dataStr = JSON.stringify(emps, null, 2);
...
// 
// HOW TO PUBLISH:
// 1. Upload this file to GitHub repo
// 2. Vercel auto-deploys in ~30 seconds
// 3. All customer card links will work!
// ============================================
window.MRK_EMPLOYEES = ${dataStr};
`;
  ...
  const a = document.createElement('a');
  a.href = url;
  a.download = 'employees-data.js';
  a.click();
  URL.revokeObjectURL(url);

  showToast(`✅ Downloaded employees-data.js with ${emps.length} employee(s)! Now upload to GitHub.`, 'success');
